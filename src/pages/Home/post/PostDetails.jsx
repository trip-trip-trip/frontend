...
//  API_BASE 끝에 슬래시(/)가 있으면 제거 (더블 슬래시 방지)
...
const PostDetail = () => {
  ...
  const fetchComments = useCallback(async () => {
    ...
    try {
      ...
      if (data.isSuccess) {
        // 2. [수정] result 안에 있는 'comments' 배열을 꺼내야 함
        // API 응답: { result: { comments: [...] } }
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.isSuccess) {
        console.error("댓글 작성 에러 응답:", data);
            ...
            <div key={c.id} className="comment-item detail-item">
              <div className="comment-line">
                <span className="comment-avatar-circle">
                  <img src={commenter.avatar_url || '/assets/default-avatar.png'} alt="" />
                  </span>
                      ...
                      <div>
                        ...
                        <span className="comment-date">
                          ...
                          >
                            ...
                            <div className="comment-body">{c.comment || c.content}</div>
                            </div>
                      </div>
                    </div>
                );
